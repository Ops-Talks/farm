...
`package-lock.json` is auto-generated by npm. Never resolve lockfile conflicts with `git checkout --theirs` or manual edits — the resulting lockfile will be inconsistent with `package.json`.

When merging Dependabot PRs or any dependency update with a `package-lock.json` conflict:

1. `git checkout --ours package-lock.json` (discard the PR's lockfile)
2. `npm install --package-lock-only` (regenerate lockfile from `package.json`, fast — no package downloads)
3. `npm run test -w apps/api` (validate)
4. `git add package-lock.json && git merge --continue`
