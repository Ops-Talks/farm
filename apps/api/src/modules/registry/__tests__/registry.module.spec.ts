  ...
  return {
    get: jest.fn((key: string) => {
      ...
      if (key === 'registry.credentials') {
        if (type === 'ecr') {
          return JSON.stringify({ accessKeyId: 'k', secretAccessKey: 's', region: 'us-east-1' });
        }
        if (type === 'gcr') {
          return JSON.stringify({ project_id: 'proj', type: 'service_account' });
        }
        if (type === 'dockerhub') {
...
describe('registryAdapterFactory', () => {
  ...
  it('should return EcrAdapter when type is ecr', () => {
    ...
    const adapter = registryAdapterFactory(config);
