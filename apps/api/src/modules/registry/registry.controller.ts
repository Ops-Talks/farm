...
import { VulnerabilityService, VulnerabilitySummary } from './vulnerability.service';
...
export class RegistryController {
  constructor(
    ...
    @Optional() @InjectQueue(VULNERABILITY_SYNC_QUEUE)
  ...
  @ApiParam({
    name: 'name',
    description: 'URL-encoded repository name (e.g. my-app or namespace%2Fmy-app)',
  })
  ...
  @ApiOperation({ summary: 'Get vulnerability scan results for a specific image tag' })
  @ApiParam({ name: 'name', description: 'URL-encoded repository name' })
  @ApiParam({ name: 'tag', description: 'Image tag' })
  ...
  async listVulnerabilities(
    ...
    if (!this.vulnService) {
      throw new ServiceUnavailableException('Vulnerability service not available');
    }
    return this.vulnService.findByComponent(componentId, severity);
  }

  @Get('components/:componentId/vulnerabilities/summary')
  @ApiOperation({ summary: 'Get vulnerability severity summary for a component' })
  @ApiParam({ name: 'componentId', description: 'Component UUID' })
  ...
  async syncVulnerabilities(
    ...
    if (!this.componentRepo) {
      throw new ServiceUnavailableException('Component repository not available');
    }
    if (!this.vulnService) {
      throw new ServiceUnavailableException('Vulnerability service not available');
    }
    ...
    if (!component.containerImage) {
      throw new BadRequestException('Component has no container image configured');
    }
