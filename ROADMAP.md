...
### FARM-E145: NestJS API — Critical Security & RBAC `TODO`
...
Address the 5 Critical and top High findings of the Next.js 16 audit. Closes the XSS surface, moves JWT off `sessionStorage`, and adds tests for the new `proxy.ts` BFF.
...
| FARM-S598 | Migrate JWT access + refresh tokens from `sessionStorage` (`apps/web/src/lib/api-client.ts:174–217, 304–319`) to `httpOnly` + `Secure` + `SameSite=Lax` cookies. Two options to evaluate (pick one, document the choice): (a) NestJS API sets the cookies on `POST /api/v1/auth/login` and `POST /api/v1/auth/refresh` directly; (b) `proxy.ts` is upgraded to a full BFF that captures the JSON tokens from the API response and sets the cookies on the way back. Update `apps/web/src/lib/api-client.ts` to stop reading tokens client-side. Update `proxy.ts` to inject the `Authorization` header from the cookie before forwarding. This unlocks Server Component data-fetching (FARM-S601). (Audit ref: Next.js C2) | `TODO` |
| FARM-S599 | Enforce CSP. In `next.config.ts:8–32`: (a) switch header from `Content-Security-Policy-Report-Only` to `Content-Security-Policy`; (b) implement per-request nonce in `proxy.ts` (`NextResponse.rewrite()` + `request.headers.set('x-nonce', nonce)`) and read it in `app/layout.tsx` to apply to `<Script>`; (c) drop `'unsafe-inline'` and `'unsafe-eval'` in production; (d) tighten `connect-src` to the API origin, Faro endpoint, and OTel collector (remove the global `https://*` wildcard and drop `ws:` in production); (e) scope CSP to non-static paths via `source: '/((?!_next/static\|_next/image\|favicon\\.ico).*)'`. Reference: https://nextjs.org/docs/app/guides/content-security-policy. (Audit ref: Next.js C3 / M20 / M21) | `TODO` |
